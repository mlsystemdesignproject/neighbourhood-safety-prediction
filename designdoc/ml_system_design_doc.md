...
 
#### 2.3.2 Этап 2. Анализ и подготовка данных и их обновлений.
...
#### 2.3.5 Этап 5. Перевод результата модели в уровень безопасности модели
- Модель выдает (Оксана)
- Принципы перевода результата модели в уровень безопасности модели

#### 2.3.6 Этап 6. Системные требования к модели. 
- Определение системных требований на вычислительные ресурсы.
- Оценка стоимости работы 1 часа и 1 месяца работы ML системы в Облаке.

#### 2.3.7 Этап 7. Утверждение метрик пилота.
(Оксана)

#### 2.3.8 Этап 8. Подготовка модели к деплою.
- Подготовить Docker 
...
#### 2.3.10 Этап 10. - Мониторинг
- Развернуть Prometheus и Grafana стек.


## 3. Ключевые фазы разработки решения задачи 

> Описание фаз развития проекта во времени

### 3.1 Первичное знакомство с задачей/данными и предварительный сервиса с тривиальной моделью

**Цель фазы.** Первичное знакомство с задачей, ее постановкой,
сбор гипотез и возможных источников данных. 
Обсуждение требований, ограничений и возможных рисков в:
- структуре данных, 
...
#### 3.1.1 Рассматриваем задачу возможные подходы к ее решению 
...
#### 3.1.2 Первичные поиск источников данных 
Для нашего проекта выбраны данные о ежемесячной преступности в районах Лондона [данные от MPS][MPS].
...
#### 3.1.3 Формулирование идей и предположений для этапа исследование 
...
#### 3.1.4 Выбор датасета и преобразование данных
В качестве данных будут использоваться [данные от MPS][MPS] о ежемесячной преступности в районах Лондона.
...
**Риски:** число пар `{Crime Type, Crime Subtype}` в статистике [MPS][MPS] равно = 55, что сильно усложняет задачу. Далее выберем существенные для проекта пары `{Crime Type, Crime Subtype}`. 

> Привести список выбранных пар

#### 3.1.5  Проведение первичного EDA, dummy вариант решения задачи 
...
#### 3.1.6 Обучение модели и валидация на отложенных тестовых данных
- Обучение и валидация модели на отложенных тестовых данных. 
- Демонстрация работы модели в jupyter notebook.

#### 3.1.7 Оформление простого интерфейса в Streamlit для модели
...
В ответ для каждого из вида преступлений (в 3.1.5 был выбран узкий список, существенный для проекта) должно выводится:
- номер недели,
- 7 векторов,
...
### 3.2 Исследование и сервис с baseline моделью

**Цель фазы.** Разработка baseline модели, исследование задачи и выбор: 
- дополнительных данных, которые могут улучшить прогноз,
- признаков(фичей),
- гранулярности данных (временного шага),
- метрик оценки бизнес эффекта (иерархия метрик),
- ML модели для baseline и MVP,
- процедуры сравнения/обновления моделей, 
- представление результата работы модели,
- архитектуры реализации ML системы,
- методов проверки изменений в данных. 

#### 3.2.1 Дополнительные данные для гипотез
...
#### 3.2.3 Выбор дополнительных данных для модели

> Итоговый выбор согласованный с заказчиком. 

#### 3.2.4 EDA новых моделей и данных

#### 3.2.5  Выбор ключевых метрик и построение модели 
> Метрики ошибки/успешности модели.
> Метрики оценки изменения в данных для выбранных датасетов.

#### 3.2.6 Выбор новых/дополнительных временных фичей 
...
#### 3.2.7 Описать процесс обновления модели

#### 3.2.8 Перевод из оценки числа преступлений в уровень безопасности
> Как осуществить перенос числа преступлений в районах города в уровень безопасности.

#### 3.2.9 Выбор способа реализации Baseline модели
> описать выбор ключевых инструментов

#### 3.2.10 Реализация Baseline модели
...
### 3.3 Совершенствование модели, реализация и развертывание MVP модели в сервис

**Цель фазы.** Подготовка и развертывание MVP модели в виде сервиса

#### 3.3.1 Оформление документации
- Sphinx (?)

#### 3.3.2 Пайплайн работы с данными

#### 3.3.3 Пайплайн работы с моделями

#### 3.3.4 Подготовка к развертыванию сервисной части проекта
- Подготовка docker образа, 
- Публикование контейнера на DockerHub,
- Оформление процесса развертывания с помощью kubernetes,
- Разворачивание сервиcа на Cloud платформе (Yandex/AWS/Azure).

#### 3.3.5 Мониторинга работы модели  
- Развернуть Prometheus и Grafana стек.



## 4. Затраты на внедрение проекта
